...
def _system_rule(**overrides):
    base = {
        "id": 1, "priority": 1, "condition_type": "table_type",
        "condition_value": "VIEW", "target_engine": "databricks",
...
def _user_rule(**overrides):
    base = {
        "id": 100, "priority": 50, "condition_type": "complexity",
        "condition_value": "high", "target_engine": "databricks",
...
class TestListRules:
    ...
    def test_returns_rules_ordered(self, mock_fetch):
        mock_fetch.return_value = [_system_rule(id=1, priority=1), _user_rule(id=100, priority=50)]
        resp = client.get("/api/routing/rules", headers=_auth_header())
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 2
...
class TestCreateRule:
    ...
    def test_creates_user_rule(self, mock_fetch):
        ...
        resp = client.post("/api/routing/rules", json={
            "priority": 60,
    ...
    def test_missing_fields_returns_422(self):
        resp = client.post("/api/routing/rules", json={"priority": 1}, headers=_auth_header())
    ...
    def test_extra_is_system_field_ignored(self, mock_fetch):
        ...
        resp = client.post("/api/routing/rules", json={
            "priority": 10, "condition_type": "x", "condition_value": "y",
            "target_engine": "duckdb", "is_system": True,
        }, headers=_auth_header())
        assert resp.status_code == 201
# ---------------------------------------------------------------------------
...
class TestUpdateRule:
    ...
    def test_updates_user_rule(self, mock_fetch):
        ...
        resp = client.put("/api/routing/rules/100", json={"priority": 99}, headers=_auth_header())
        assert resp.status_code == 200
        assert resp.json()["priority"] == 99
    @patch("main.db.fetch_one", return_value=None)
    def test_not_found(self, _):
        resp = client.put("/api/routing/rules/999", json={"priority": 1}, headers=_auth_header())
        assert resp.status_code == 404
    @patch("main.db.fetch_one", return_value=_system_rule())
    def test_system_rule_returns_403(self, _):
        resp = client.put("/api/routing/rules/1", json={"priority": 99}, headers=_auth_header())
...
class TestResetRules:
    ...
    def test_reset_returns_system_rules(self, mock_exec, mock_fetch):
        system_rules = [
            _system_rule(id=i, priority=i) for i in range(1, 6)
        ]
