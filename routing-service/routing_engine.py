...
def route_query(
    ...
    """Run the routing pipeline and return a decision.

    Pipeline order (first match wins):
      0. Error check
      1. System hard rules (always evaluated)
      2. Forced mode (duckdb/databricks)
      3. User-defined rules
      4. ML model (stub — always None in Phase 8)
    ...
    # 1. System hard rules (always evaluated, even with forced mode)
    ...
    # 4. ML model stub (Phase 8: always None)
    ml_decision = None
    ...
    if score < 5 and all(
        ...
        return RoutingDecision(
            engine="duckdb",
            stage="FALLBACK",
            reason="Low complexity, all tables DuckDB-compatible",
            complexity_score=score,
        )
