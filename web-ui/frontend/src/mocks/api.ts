import type {
  HealthStatus, DatabricksSettings, DatabricksCredentials, DatabricksConnectResponse,
...
// ---- Mutable state ----
let isDatabricksConfigured = false;
let databricksHost: string | null = null;
let databricksUsername: string | null = null;
...
let models: Model[] = [
  { id: 1, linked_engines: ["duckdb:2gb-2cpu", "databricks:serverless-2xs", "duckdb:8gb-4cpu"], model_path: "/models/model_001.joblib", accuracy_metrics: { r_squared: 0.87, mae_ms: 45 }, is_active: false, created_at: "2026-03-10T14:30:00Z" },
];
...
export const mockApi = {
  // Health
  async getHealthServices(): Promise<HealthStatus> {
    await delay(100);
    return {
      web_ui: { status: "connected" },
      routing_service: { status: "connected" },
      postgresql: { status: "connected" },
      duckdb_worker: { status: "connected" },
      databricks: { status: isDatabricksConfigured ? "connected" : "not_configured" },
    };
  },
  ...
  // Databricks settings
  async getDatabricksSettings(_token: string): Promise<DatabricksSettings> {
    await delay(200);
    return { configured: isDatabricksConfigured, host: databricksHost || undefined, username: databricksUsername || undefined, warehouse_id: selectedWarehouseId };
  },

  async saveDatabricksSettings(_token: string, creds: DatabricksCredentials): Promise<DatabricksConnectResponse> {
    await delay(800);
    isDatabricksConfigured = true;
    databricksHost = creds.host;
    databricksUsername = "admin@databricks.com";
    return { status: "connected", host: creds.host, username: "admin@databricks.com" };
  },

  async getWarehouses(_token: string): Promise<Warehouse[]> {
    await delay(400);
    return [
      { id: "wh-1", name: "Starter Warehouse", state: "RUNNING" },
      { id: "wh-2", name: "Production Warehouse", state: "RUNNING" },
      { id: "wh-3", name: "Dev Warehouse", state: "STOPPED" },
    ];
  },

  async saveWarehouse(_token: string, warehouseId: string): Promise<{ status: string }> {
    await delay(200);
  ...
  // Catalog browser
  async getCatalogs(_token: string): Promise<CatalogInfo[]> {
    await delay(300);
    return [{ name: "delta_router_dev" }];
  },

  async getSchemas(_token: string, _catalog: string): Promise<SchemaInfo[]> {
    await delay(300);
    return [
  ...
  async getTables(_token: string, catalog: string, schema: string): Promise<TableInfo[]> {
    await delay(400);
    return tableData[`${catalog}.${schema}`] || [];
  },

  // Collections
  async getCollections(_token: string): Promise<Collection[]> {
    await delay(200);
    return collections.map(({ queries: _q, ...c }) => c);
  },

  async getCollection(_token: string, id: number): Promise<CollectionWithQueries> {
    await delay(200);
  ...
  async createCollection(_token: string, name: string, description: string): Promise<Collection> {
    await delay(300);
  ...
  async updateCollection(_token: string, id: number, data: Partial<Collection>): Promise<Collection> {
    await delay(200);
    const c = collections.find(c => c.id === id);
  ...
  async deleteCollection(_token: string, id: number): Promise<void> {
    await delay(200);
    collections = collections.filter(c => c.id !== id);
  },

  async addQuery(_token: string, collectionId: number, queryText: string): Promise<Query> {
    await delay(200);
  ...
  async updateQuery(_token: string, collectionId: number, queryId: number, queryText: string): Promise<Query> {
    await delay(200);
    const c = collections.find(c => c.id === collectionId);
    if (!c) throw new Error("Not found");
    const q = c.queries.find(q => q.id === queryId);
    if (!q) throw new Error("Not found");
    q.query_text = queryText;
    return { ...q };
  },

  async deleteQuery(_token: string, collectionId: number, queryId: number): Promise<void> {
    await delay(200);
    const c = collections.find(c => c.id === collectionId);
    if (!c) throw new Error("Not found");
  ...
  // Query Execution
  async executeQuery(_token: string, sql: string, routingMode: string): Promise<QueryExecutionResult> {
    ...
    if (sqlLower.includes("customer") && !sqlLower.includes("customer_pii") && !sqlLower.includes("customer_demographics")) {
      ...
      return {
        correlation_id: `q-${Date.now()}`,
        routing_decision: { engine, engine_display_name: engineName, stage, reason, complexity_score: complexity },
    ...
    // Append to query logs
    queryLogs.unshift({
      correlation_id: correlationId, timestamp: new Date().toISOString().replace("T", " ").slice(0, 19),
      query_text: sql, engine, engine_display_name: engineName, status: "success", latency_ms: execTime, cost_usd: cost,
    });
  ...
  // Benchmarks
  async getBenchmarks(_token: string, collectionId: number): Promise<BenchmarkSummary[]> {
    await delay(200);
    return benchmarks.filter(b => b.collection_id === collectionId).map(({ warmups: _w, results: _r, ...rest }) => rest);
  },

  async getBenchmark(_token: string, id: number): Promise<BenchmarkDetail> {
    await delay(300);
  ...
  async createBenchmark(_token: string, collectionId: number, _catalogEngineIds: number[]): Promise<BenchmarkSummary> {
    await delay(500);
  ...
  async deleteBenchmark(_token: string, id: number): Promise<void> {
    await delay(200);
    benchmarks = benchmarks.filter(b => b.id !== id);
  },

  // Engine Catalog
  async getEngineCatalog(_token: string): Promise<EngineCatalogEntry[]> {
    await delay(200);
    return JSON.parse(JSON.stringify(engineCatalog));
  },

  async toggleEngineCatalogEntry(_token: string, id: number, enabled: boolean): Promise<EngineCatalogEntry> {
    await delay(200);
  ...
  async resetEngineCatalog(_token: string): Promise<EngineCatalogEntry[]> {
    await delay(300);
  ...
  // Engine Preferences
  async getEnginePreferences(_token: string): Promise<EnginePreference[]> {
    await delay(200);
    return [
      { engine_id: "duckdb:2gb-2cpu", display_name: "DuckDB 2GB/2CPU", engine_type: "duckdb", preference_order: 1 },
      { engine_id: "databricks:serverless-2xs", display_name: "Serverless 2X-Small", engine_type: "databricks_sql", preference_order: 2 },
      { engine_id: "duckdb:8gb-4cpu", display_name: "DuckDB 8GB/4CPU", engine_type: "duckdb", preference_order: 3 },
    ];
  },

  async saveEnginePreferences(_token: string, _preferences: EnginePreference[]): Promise<void> {
    await delay(200);
  },

  // Routing Rules
  async getRoutingRules(_token: string): Promise<RoutingRule[]> {
    await delay(200);
    return JSON.parse(JSON.stringify(routingRules));
  },

  async createRoutingRule(_token: string, rule: Omit<RoutingRule, "id">): Promise<RoutingRule> {
    await delay(200);
  ...
  async deleteRoutingRule(_token: string, id: number): Promise<void> {
    await delay(200);
    routingRules = routingRules.filter(r => r.id !== id);
  },

  async toggleRoutingRule(_token: string, id: number, enabled: boolean): Promise<RoutingRule> {
    await delay(200);
  ...
  async resetRoutingRules(_token: string): Promise<RoutingRule[]> {
    await delay(300);
    routingRules = [
      { id: 1, priority: 1, condition_type: "table_type", condition_value: "VIEW", target_engine: "databricks", is_system: true, enabled: true },
      { id: 2, priority: 2, condition_type: "has_governance", condition_value: "row_filter", target_engine: "databricks", is_system: true, enabled: true },
  ...
  // Routing Settings
  async getRoutingSettings(_token: string): Promise<RoutingSettings> {
    await delay(200);
    return { ...routingSettings };
  },

  async saveRoutingSettings(_token: string, settings: RoutingSettings): Promise<RoutingSettings> {
    await delay(200);
  ...
  async trainModel(_token: string): Promise<Model> {
    await delay(3000);
    const m: Model = {
      id: nextModelId++, linked_engines: ["duckdb:2gb-2cpu", "databricks:serverless-2xs"],
  ...
  async activateModel(_token: string, id: number): Promise<Model> {
    await delay(200);
  ...
  async deactivateModel(_token: string, id: number): Promise<Model> {
    await delay(200);
    const m = models.find(m => m.id === id);
  ...
  async deleteModel(_token: string, id: number): Promise<void> {
    await delay(200);
    models = models.filter(m => m.id !== id);
  },

  // Query Log
  async getQueryLogs(_token: string, engineFilter?: string): Promise<LogEntry[]> {
    await delay(200);
    let logs = [...queryLogs];
