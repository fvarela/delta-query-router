...
- [ ] **Phase 10 scope definition:** Benchmark Infrastructure phase needs PRD. Core scope: collections CRUD, benchmark execution (run query on each engine, capture execution_time_ms), results storage in PostgreSQL, storage latency probes (ODQ-9), benchmark UI wiring (replace mocks in `src/mocks/api.ts`). ML model training pipeline may be a separate phase or included depending on complexity.
...
- [ ] **Phase 10 - Benchmark Infrastructure:** Collections CRUD, benchmark execution engine (run each query on each enabled engine, capture execution_time_ms per ODQ-14), results storage in PostgreSQL, storage latency probes (ODQ-9), benchmark detail UI. Replaces mock API calls in `src/mocks/api.ts` with real backend endpoints. Cold-start is captured separately in `benchmark_engine_warmups`. ML model training pipeline may be included or deferred to a follow-up phase depending on scope. PRD pending.
- [ ] **Phase 11 - Azure AKS Deployment:** Deploy the full Delta Router stack to Azure using Terraform IaC. **Motivation:** DuckDB worker latency is ~10x Databricks when running locally — co-locating workers in the same Azure region as the data eliminates network round-trip overhead, which is the core value proposition of the project. **Infrastructure:** Resource Group, AKS cluster, ACR (attached to AKS via managed identity for native pull access), networking (VNet, load balancer, public IP). PostgreSQL remains a StatefulSet in AKS (migration to Azure Database for PostgreSQL Flexible Server is straightforward later — connection string change + Terraform diff). **IaC:** Terraform with `azurerm` provider (cloud-agnostic choice; also has a `databricks` provider for future workspace automation). Existing K8s manifests in `k8s/` converted to Helm chart templates. **CI/CD:** Pipeline to build images, push to ACR, deploy to AKS. See `infrastructure/` module for directory structure and IaC split details.
