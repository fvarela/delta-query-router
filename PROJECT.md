...
- [ ] **Phase 16 - TPC-DS Query Collections & End-to-End Validation:** Auto-generate TPC-DS query collections (99 standard queries per scale factor) after TPC-DS data creation, with table references rewritten to three-part names. Bridges the gap between data creation (Phase 14) and benchmark/model training (Phase 10/13). Auto-create `tpcds`-tagged read-only collection on data ready, cascade delete on data deletion. Frontend triggers collection reload after wizard success. PRD: `.taskmaster/docs/phase16-tpcds-queries-e2e-validation.md`.
