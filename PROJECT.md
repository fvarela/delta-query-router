...
- [ ] **Phase 19 - Ephemeral Databricks Warehouses & DuckDB Resource Bump:** Two pillars: (1) Ephemeral warehouse lifecycle — auto-create serverless warehouses matching engine `cluster_size` during benchmarks (create → wait RUNNING → execute → delete in `finally`), tagged `delta-router-managed` for safety, orphan cleanup on startup, `auto_stop_mins=5` safety net. New `ephemeral_warehouses.py` module. (2) DuckDB resource bump — duckdb-1 from 1 CPU/1 GB to 2 CPU/2 GB in schema seed, K8s manifest, and Helm values. Also: 3 new Databricks engine seeds (Medium, Large, X-Large). PRD: `.taskmaster/docs/phase19-ephemeral-warehouses-duckdb-bump.md`.
