...
- [ ] **Phase 18 - System Cleanup & Local End-to-End Validation:** Four pillars: (1) Bug fixes — `duckdb-1` k8s_service_name, Helm schema.sql sync, Terraform secret names. (2) Remove user rules — dropped `is_system` column from `routing_rules` (user rule CRUD was never built). (3) SDK alignment — `profile_id` on execute, management methods (list_profiles, get_profile, list_engines, get_routing_settings). (4) Deploy & validate on Minikube.
