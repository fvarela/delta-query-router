...
**Schema:**
- `routing_rules`: id (serial PK), priority (int), condition_type (text), condition_value (text), target_engine (text), enabled (bool)
- 5 governance rules seeded by migration (views, RLS, column masking, foreign tables, no external access). Read-only — can only be toggled on/off
...
- **Implementation (2026-04-14):** AST features and table metadata are stored in `query_features` table (keyed on `query_id`), pre-computed eagerly (AST at query insert via `query_features.compute_and_store()`, table metadata snapshot at benchmark run time via `_snapshot_table_metadata()`). Trainer JOINs `benchmark_results` with `query_features` instead of re-parsing SQL. Table metadata variability across runs is a known factor — recommended practice is training on fixed datasets (TPC-DS). See Ongoing Topics for full rationale. Existing queries are backfilled on server startup.
...
- [x] **Phase 18 — System Cleanup & Local End-to-End Validation (2026-04-11):** Resolved scope. Four pillars: (1) **Bug fixes**: fix `duckdb-1` k8s_service_name (`duckdb-worker` → `duckdb-worker-small`), sync Helm `files/schema.sql`, fix Terraform secret name mismatch. (2) **Remove user rules**: drop `is_system` column from `routing_rules` table (user-defined rule CRUD was never implemented — only the column existed as scaffolding). Routing pipeline unchanged: hard rules → ML model → fallback. (3) **SDK alignment**: add `profile_id` on `execute()`, add management methods (`list_profiles()`, `get_profile()`, `list_engines()`, `get_routing_settings()`), update tests. (4) **Deploy & validate**: fresh Minikube deployment, all 5 services healthy, query execution through full proxy path verified, query_features backfill confirmed (99 TPC-DS rows). No issues found.
- [x] **AST analysis data plan for ML training (2026-04-14):** Resolved. Current gap: `benchmark_results` only stores `execution_time_ms` — no AST features or table metadata. ML trainer (`model_trainer.py`) re-parses SQL at training time and passes `table_metadata={}` (empty), causing train/serve skew for size-based features. **Decision:** pre-compute and store eagerly. New `query_features` table keyed on `query_id` stores all 10 AST features (from `query_analyzer.py`) at query insert time, plus table metadata snapshot (`total_data_bytes`, `max_table_size_bytes`) captured at benchmark execution time. Trainer JOINs `benchmark_results` with `query_features` instead of re-parsing. New module: `query_features.py` with `compute_and_store()`, `compute_and_store_batch()`, `update_table_metadata()`, `backfill_all()`. Wired into `collections_api.add_query()`, `tpcds_api._create_tpcds_collection()`, `benchmarks_api._run_benchmark_inner()`. Backfill runs on server startup. **Table metadata variability:** live tables can grow/shrink between benchmark runs — recommended practice is to train models on fixed datasets (TPC-DS) where table sizes are constant. Per-run table metadata snapshots are captured for future use but not currently factored into training.
...
- [x] **Databricks warmup accuracy (2026-04-14):** Resolved. Same root cause as DuckDB warmup — `SELECT 1` only proved the warehouse was reachable but didn't exercise the real session initialization costs (warehouse auto-start/scale-up, catalog metadata resolution, internal cache warming). Q1 was absorbing ~2x overhead compared to Q2. Fixed: warmup now runs `SELECT 1 FROM <first_table> LIMIT 1` which forces full query path initialization, so `cold_start_time_ms` captures the true one-time cost and per-query `execution_time_ms` reflects actual query performance.
...
- [x] **Routing Decision View improvements (2026-05-03):** Resolved. Three changes implemented: (1) Removed formula explanation line. (2) Unified scoring table showing all model-supported engines — filtered engines greyed out with reason tags (e.g., "No warehouse mapped"). (3) Info-icon tooltips on TIER, PERF, COST, SCORE headers with structured content (bold title → mono formula box → explanation). Additional improvements from feedback: PERF/COST columns now show raw 0–1 normalized values (not weighted contributions), tooltip text uses consistent terminology matching column names, no emoji in tooltips (was rendering as `[]`), tooltip inherits reset to prevent all-bold in Score column.
- [ ] **DuckDB engine tiers & lifecycle (2026-05-04):** Phase 21 PRD created. Five fixed tiers (XSmall 2GB through XLarge 32GB), lifecycle modes (always-on / on-demand with idle timeout), DuckDB cold start measurement for on-demand engines, routing integration with cold start penalty. UI prototype first (Task 1 — `@build` with `frontend-design` skill), then backend. Current `duckdb-1` replaced, existing models deleted. Tab naming TBD — Benchmarks tab may need renaming since it will host engine management.
- [ ] **Standalone cold start measurement (2026-05-03):** Cold start times are currently only measured during benchmark warmup (`benchmark_engine_warmups` table). This ties an engine property to benchmark artifacts and means engines without benchmarks have no cold start data (falls back to hardcoded defaults). **Decision:** Separate cold start measurement from benchmarking. New `engine_cold_starts` table (engine_id, cold_start_ms, measured_at) — stores latest value per engine type. Measurement approach: run a dummy query (`SELECT 1 FROM <real_table> LIMIT 1`) twice, save Q1−Q2 as cold start (captures startup + first-query initialization overhead). **Databricks:** create ephemeral warehouse → stop it → start + Q1 → Q2 → save Q1−Q2 → delete warehouse. Creation time excluded — only startup time measured. Reuses ephemeral warehouse mechanism from Phase 19. **DuckDB:** always-on, so cold start is per-query credential vending delta (likely ~0ms). Measure Q1−Q2 on a real table for consistency. Needs a real UC table (not `SELECT 1`) because the overhead is in credential vending, not DuckDB itself. **UI:** "Measure Cold Start" button in right panel Routing Settings per engine. **Smart routing gate:** no cold start measurement = engine disabled for smart routing. **Benchmarks continue measuring warmup** and also update the cold start table (keeps data fresh). Overlaps with Phase 19 ephemeral warehouses.
- [ ] **Ephemeral Databricks warehouses for benchmarking (2026-04-14):** Current limitation: benchmarking against different Databricks warehouse sizes requires manually pre-creating warehouses in the Databricks console. The 3 pre-seeded Databricks engine slots (2X-Small, X-Small, Small) all fall back to the single global `_warehouse_id` — you can't benchmark across different sizes. **Decision:** create-per-benchmark (Option A). When a benchmark targets a Databricks engine without a `warehouse_id` in its config, the benchmark runner creates a serverless warehouse matching the engine's `cluster_size`, tags it `delta-router-ephemeral-{run_id}`, waits for RUNNING, runs the benchmark, then deletes it in a `finally` block. Safety: never delete a warehouse without the `delta-router-managed` tag. Extra safety net: `auto_stop_mins=5`. Orphan cleanup on startup: scan for `delta-router-ephemeral-*` warehouses and delete them. Only during benchmarks, not ad-hoc queries. Also bumping default DuckDB Small resources from 1 CPU/1 GB to 2 CPU/2 GB.
...
- [x] **Phase 18 - System Cleanup & Local End-to-End Validation:** Four pillars: (1) Bug fixes — `duckdb-1` k8s_service_name, Helm schema.sql sync, Terraform secret names. (2) Remove user rules — dropped `is_system` column from `routing_rules` (user rule CRUD was never built). (3) SDK alignment — `profile_id` on execute, management methods (list_profiles, get_profile, list_engines, get_routing_settings). (4) Deploy & validate on Minikube — all services healthy, end-to-end query execution verified, query_features backfill confirmed.
- [x] **Phase 19 - Ephemeral Databricks Warehouses & DuckDB Resource Bump:** Two pillars: (1) Ephemeral warehouse lifecycle — auto-create serverless warehouses matching engine `cluster_size` during benchmarks (create → wait RUNNING → execute → delete in `finally`), tagged `delta-router-managed` for safety, orphan cleanup on startup, `auto_stop_mins=5` safety net. New `ephemeral_warehouses.py` module. (2) DuckDB resource bump — duckdb-1 from 1 CPU/1 GB to 2 CPU/2 GB in schema seed, K8s manifest, and Helm values. Also: 3 new Databricks engine seeds (Medium, Large, X-Large). PRD: `.taskmaster/docs/phase19-ephemeral-warehouses-duckdb-bump.md`.
- [ ] **Phase 20 - Standalone Cold Start Measurement (partially superseded by Phase 21 for DuckDB engines):** Decouples cold start measurement from benchmarking. New `engine_cold_starts` table stores latest cold start per engine. Measurement: dummy query on real UC table — Q1 minus Q2 captures startup + first-query init overhead. Databricks: ephemeral warehouse (reuses Phase 19 module) → stop → start → Q1 → Q2 → save delta → delete. DuckDB: Q1 → Q2 on real table (credential vending overhead). UI: "Measure Cold Start" button per engine in Routing Settings. Gate: no cold start data = engine disabled for smart routing. Benchmarks continue measuring warmup and also update the cold start table. PRD: `.taskmaster/docs/phase20-standalone-cold-start.md`.
- [ ] **Phase 21 - DuckDB Engine Tiers & Lifecycle Management:** Flexible DuckDB engine sizing and lifecycle. Five fixed tiers: XSmall (2GB) / Small (4GB) / Medium (8GB) / Large (16GB) / XLarge (32GB). Each engine configurable as always-on (replicas=1, cold start 0ms) or on-demand (scale to 0 when idle, configurable timeout: 5/15/30/60 min). Cold start measurement for on-demand engines (scale 0→1 + dummy query). Router scores on-demand engines with cold start penalty — only starts them if they win scoring. UI prototype first (mock data in Benchmarks tab area), then schema + K8s manifests + lifecycle API + wiring. Replaces current `duckdb-1` (2GB) with 5 proper tiers. Existing ML models deleted — fresh training required. PRD: `.taskmaster/docs/phase21-duckdb-engine-tiers-lifecycle.md`.
...
- DuckDB engine tiers: XSmall (2GB) / Small (4GB) / Medium (8GB) / Large (16GB) / XLarge (32GB) — fixed tiers tied to ML model training
- DuckDB engine lifecycle: always-on (replicas=1) or on-demand (scale to 0 when idle, configurable timeout)
